...
for i in range(1000):
    ...
    if i % 100 == 0:
        print i
        print(
        sess.run(accuracy, feed_dict={x_: numpy.subtract(1, mnist.test.images[:1000]), y_: mnist.test.labels[:1000], keep_prob: 0.5}))
    train_step.run(feed_dict={x_: numpy.subtract(1, batch_xs), y_: batch_ys, keep_prob: 0.5})
